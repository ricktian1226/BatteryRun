...
REM goods
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd% -e "select id as Id, malltype as MallType, mallsubtype as MallSubType, posindex as PosIndex, discount as Discount, price as Price, iapid as IapID, items as Items, IFNULL(amountperuser, '') as AmountPerUser, IFNULL(amountperround,'') as AmountPerRound, IFNULL(amountperday, '') as AmountPerDay, if(bestdeal=0,'','TRUE') as BestDeal, if(tesell=0,'','TRUE') as TeSell, expireddate as ExpiredDate, if(valid=0, 'FALSE', 'TRUE')  as Valid, icon as Icon, IFNULL(name, '')  as Name, IFNULL(description, '') as Description, label as Label, IFNULL(displayids, '') as DisplayIds, IFNULL(giftids, '') as GiftIds, multiple as Multiple from %dbname%.goods" > "%outdir%/商品配置信息.txt"
...
REM runeconfig
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd% -e "select propid as ID, value as Value, IFNULL(expiredlimitation,'') as ExpiredLimitation from %dbname%.runeconfig" > "%outdir%/系统道具数值配置.txt"
...
REM newaccountprop
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd%  -e "select source as Source,propitems as PropItems,type as Type,,mailid as MailID from %dbname%.newaccountprop"> "%outdir%/首次登陆礼包配置信息.txt"


REM checkpointunlockgoodsconfig
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd% -e "select checkpointid as CheckPointId,unlockgoodsid as UnlockGoodsId from %dbname%.checkpointunlockgoodsconfig" > "%outdir%/关卡解锁商品配置.txt"

REM shareaward 
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd% -e "select id as Id,counter as Counter,rewards as Rewards from %dbname%.shareaward" > "%outdir%/分享奖励配置.txt"

REM shareactivity
mysql -h%host% -P%port% -u%dbuser% -p%dbpasswd% -e "select id as Id,type as Type,goalvalue as GoalValue, dailylimit as DailyLimit, dispensetype as DispenseType, activitybegintime as ActivityBeginTime, activityendtime as ActivityEndTime,restart as Restart,valid as Valid,mailid as MailId,awardsbegintime as AwardsBegintime,awardsendtime as AwardsEndtime  from %dbname%.shareactivity" >"%outdir%/分享活动配置.txt"
REM 备份数据库
backup_config.bat