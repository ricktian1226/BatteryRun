...
func ProcessMessage(subject string, req_data []byte, req_type reflect.Type, resp_type reflect.Type, handle xynatsservice.ApiHandler) (resp_data []byte, err error) {
	...
	xylog.Info("[%s] Process Message start", subject)
	defer xylog.Info("[%s] Process Message end", subject)
	...
	if err != nil {
		xylog.Error("Error pre-processing : %s", err.Error())
		goto ErrorHandler
	}
...
ErrorHandler:
	...
	if err != nil {
		xylog.Error("Error post-processing : %s", err.Error())
